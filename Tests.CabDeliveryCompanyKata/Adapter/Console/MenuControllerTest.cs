...
public class MenuControllerTest
{
    ...
    public void CanDisplayLoginMenu()
    {
        ...
        var displayMenu = menuController.DisplayMenu(null);
        
        Assert.Equal("Please choose a selection from the list: ", displayMenu.First());
        Assert.Equal("0. Exit", displayMenu.Skip(1).First());
        Assert.Equal("8. Register New User", displayMenu.Skip(2).First());
        Assert.Equal("9. Login Existing User", displayMenu.Skip(3).First());
    }
    [Fact]
    public void CanMutateDisplayMenu()
    {
        ...
        cabContext.ChangeTracker.Clear();
        var loginUser = dispatchController.LoginUser(1);
        cabContext.ChangeTracker.Clear();
        dispatchController.AddCab(1);
        cabContext.ChangeTracker.Clear();
        
        var displayMenu = menuController.DisplayMenu(1);
        
        Assert.Equal("1", loginUser);
        Assert.Equal("Please choose a selection from the list: ", displayMenu.First());
        Assert.Equal("0. Exit", displayMenu.Skip(1).First());
